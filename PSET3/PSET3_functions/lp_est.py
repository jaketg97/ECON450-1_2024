...
def LP_estimation(dat):
    ...
    y = LP_data["y_gross"].copy()
    # lp_first_stage = sm.OLS(LP_data["y_gross"], sm.add_constant(LP_data[phi_vars + ["l"]])).fit()
    # beta_hat_l = lp_first_stage.params["l"]
    # LP_data["y_gross_hat"] = lp_first_stage.predict(sm.add_constant(LP_data[phi_vars + ["l"]]))
    LP_data["y_gross_hat"] = jnp_reg_predict(y, X)
    beta_hat_stage1 = jnp_reg(y, X)
    beta_hat_l = beta_hat_stage1[len(beta_hat_stage1)].item()
    LP_data["phi_prediction"] = LP_data["y_gross_hat"] - beta_hat_l * LP_data["l"]
    ...
    def LP_GMM_val_grad(params, data):
        ...
        ksi_epsilon = dat[:, column_indices["y_gross"]] - beta_hat_l * dat[:, column_indices["l"]] - beta_k * dat[:, column_indices["k"]] - beta_m * dat[:, column_indices["m"]] - omega_hat
    ...
    return(beta_k, beta_m, beta_hat_l)